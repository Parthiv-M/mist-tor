...
const clearanceEdit = require('./../../middleware/clearanceEdit');

// create a news article
router.post('/news/create', auth, async (req, res) => {
  try {
...
router.delete('/news/delete/:headline', auth, async (req, res) => {
  try {
    const boardUser = await req.user;

    if (boardUser.position == 'WebHead' || boardUser.position == 'HR') {
      try {
...
router.post('/addwriters', auth, async (req, res) => {
